...
Yerel durum (29 Haziran 2026): PostgreSQL 17.10 üzerinde dört migration ve altı gerçek veritabanı integration testi geçti. `pnpm test:integration` hold–hold, uygulama servisi hold yarışı, hold–randevu, randevu–randevu ve farklı uzman senaryolarını doğrular. Canlıya çıkıştan önce seçilecek yönetilen PostgreSQL/CI ortamında aynı paket yeniden çalıştırılmalıdır.
