...
await patchFile("components/workspaces/RequestsServicesViews.tsx", (initialSource) => {
  return replaceRegexRequired(
    initialSource,
    /  if \(selectedItemId === 'gonderim-gecmisi'\) \{[\s\S]*?\n  \}\n\n  return \(/,
    `  if (selectedItemId === 'gonderim-gecmisi') {
    const query = normalized(filter);
    const rows = [...(data?.outbox ?? [])]
      .filter((row) => normalized(String(row.eventType) + ' ' + String(row.aggregateType) + ' ' + String(row.aggregateId) + ' ' + String(row.status)).includes(query))
      .sort((left, right) =>
        (new Date(left.createdAt).getTime() - new Date(right.createdAt).getTime()) *
        (sortDirection === 'asc' ? 1 : -1),
      );
    return rows.length === 0 ? (
      <EmptyState title="Gönderim kaydı bulunamadı" text="Gerçek gönderim kuyruğu kayıtları oluştuğunda burada gösterilir." />
    ) : (
      <section className="overflow-hidden rounded-[2rem] border border-black/[0.07] bg-white/88">
        {rows.map((row, index) => (
          <article key={row.id} className={'grid grid-cols-[1fr_220px_auto] gap-4 px-5 py-4 ' + (index > 0 ? 'border-t border-black/[0.05]' : '')}>
            <div><strong className="block text-[11px] font-black">{row.eventType}</strong><span className="mt-1 block text-[9px] font-semibold text-gray-400">{row.aggregateType} · {row.aggregateId}</span></div>
            <div><span className="block text-[10px] font-bold text-gray-600">{formatDate(row.createdAt, true)}</span>{row.processedAt && <span className="mt-1 block text-[8.5px] font-semibold text-gray-400">İşlendi: {formatDate(row.processedAt, true)}</span>}</div>
...
await patchFile("components/WorkspacePanel.tsx", (initialSource) => {
  ...
  source = replaceRequired(
    ...
    `                      {/* Payment badge is only shown when there is a meaningful appointment-level value. */}
                      {app.payment && (
                        <span className={\`px-2.5 py-1 rounded-full text-[9px] font-black tracking-widest uppercase ${
                          app.payment === 'Ödendi'
                            ? 'bg-[#eafda8]/20 text-lime-900 border border-lime-200/40'
                            : 'bg-amber-500/10 text-amber-800'
                        }\`}>
                          {app.payment}
                        </span>
                      )}`,
    "Hide meaningless appointment payment badge",
  );
  return source;
});
