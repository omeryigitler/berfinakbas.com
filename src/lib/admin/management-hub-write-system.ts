...
export async function writeSystemAction(
  ...
  if (action.action === "SAVE_SETTING") {
    ...
    return NextResponse.json({
      data: await saveSetting(session, action.key, action.value, action.reason),
    });
  }
  if (action.action === "SAVE_PDF_RESOURCES") {
    ...
    const saved = await saveSetting(
      ...
      action.resources,
      "PDF ve kaynak metadata kayıtları yönetim panelinden güncellendi.",
    );
  ...
  if (action.action === "UPDATE_SERVICE_STATUS") {
    ...
    const updated = await database.service.update({
      data: {
        ...
        ...(action.status === "INACTIVE" ? { publicVisible: false } : {}),
      },
    ...
    await database.auditLog.create({
      data: {
        ...
        afterSummary: { name: updated.name, status: updated.status },
        correlationId,
        entityId: updated.id,
        entityType: "SERVICE",
  ...
  if (action.action === "UPDATE_USER_STATUS") {
    ...
    if (action.userId === session.user.id && action.status === "SUSPENDED")
      return NextResponse.json(
        { error: "Kendi yönetici hesabınızı askıya alamazsınız." },
        { status: 400 },
      );
    ...
    const updated = await database.$transaction(async (transaction) => {
      ...
      await transaction.auditLog.create({
        data: {
          ...
          actorType: "USER",
          actorUserId: session.user.id,
          afterSummary: { status: client.status },
          correlationId,
          entityId: client.id,
          entityType: "CLIENT",
          reason: "MANAGEMENT_HUB_RESTORE",
        },
      });
      return client;
    });
    ...
    const updated = await database.$transaction(async (transaction) => {
      ...
      await transaction.auditLog.create({
        data: {
          ...
          actorType: "USER",
          actorUserId: session.user.id,
          afterSummary: { status: service.status },
          correlationId,
          entityId: service.id,
          entityType: "SERVICE",
          reason: "MANAGEMENT_HUB_RESTORE",
