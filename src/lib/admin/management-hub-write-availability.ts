...
export async function writeAvailabilityAction(
  ...
  if (action.action === "SAVE_AVAILABILITY_RULES") {
    ...
    if (!practitioner)
      return NextResponse.json({ error: "Aktif terapist kaydı bulunamadı." }, { status: 404 });
    for (const rule of action.rules) {
      if (rule.start >= rule.end)
        return NextResponse.json(
          { error: "Çalışma saati başlangıcı bitişten önce olmalıdır." },
          { status: 400 },
        );
    }
    await database.$transaction(async (transaction) => {
      ...
      await transaction.auditLog.create({
        data: {
          ...
          afterSummary: { count: action.rules.length },
          correlationId,
          entityId: practitioner.id,
          entityType: "PRACTITIONER",
          reason: "MANAGEMENT_HUB_UPDATE",
        },
      });
    });
  ...
  if (action.action === "CREATE_AVAILABILITY_EXCEPTION") {
    ...
    if (action.type !== "CLOSED" && (!action.start || !action.end || action.start >= action.end)) {
      return NextResponse.json(
        { error: "Özel veya bloklu zaman için geçerli başlangıç ve bitiş saati gerekir." },
        { status: 400 },
      );
    }
    const created = await database.$transaction(async (transaction) => {
      ...
      await transaction.auditLog.create({
        data: {
          ...
          actorType: "USER",
          actorUserId: session.user.id,
          afterSummary: { date: action.date, type: action.type },
          correlationId,
  ...
  if (action.action === "SET_AVAILABILITY_EXCEPTION_STATUS") {
    ...
    const updated = await database.$transaction(async (transaction) => {
      const result = await transaction.availabilityException.updateMany({
        data: { status: action.status },
        where: { id: action.id, practitionerId: practitioner.id },
      });
      if (result.count > 0)
        await transaction.auditLog.create({
          data: {
            action: "availability.exception.status.updated",
            actorType: "USER",
            actorUserId: session.user.id,
            afterSummary: { status: action.status },
            correlationId,
            entityId: action.id,
            entityType: "AVAILABILITY_EXCEPTION",
            reason: "MANAGEMENT_HUB_UPDATE",
