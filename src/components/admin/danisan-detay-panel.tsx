...
export default function DanisanDetayPanel({
  ...
  const latestService =
    detail?.nextAppointment?.serviceNameSnapshot ??
    detail?.appointments[0]?.serviceNameSnapshot ??
    "Henüz hizmet yok";
  ...
  if (!selectedDanisanId) {
    return (
      <section className={styles.panel}>
        <div className={styles.loadingState}>Bir danışan seçin.</div>
      </section>
    );
  }
  ...
  if (loading && !detail) {
    return (
      <section className={styles.panel}>
        <div className={styles.loadingState}>Danışan bilgileri yükleniyor...</div>
      </section>
    );
  }
    ...
    return (
      <section className={styles.panel}>
        <div className={styles.errorState}>
          ...
          <button className={styles.btnPrimary} onClick={() => void loadDetail()} type="button">
            Yeniden dene
          </button>
        </div>
      </section>
    ...
    <section className={styles.panel}>
      <div className={styles.topSection}>
        <div className={styles.toolbar}>
          ...
          <button className={styles.toolBtn} onClick={onNew} type="button">
            ➕ Yeni
          </button>
          <button
            className={styles.toolBtn}
          ...
          <button className={styles.toolBtn} onClick={() => void loadDetail()} type="button">
            🔄 Yenile
          </button>
          <button
            className={styles.toolBtn}
          ...
          <button className={styles.toolBtn} onClick={() => window.print()} type="button">
            📊 Raporla
          </button>
        </div>
            ...
            <div className={styles.headerInfo}>
              ...
              <div className={styles.headerBadges}>
                <span className={styles.badge}>
                  {detail.type === "CHILD" ? "ÇOCUK DANIŞAN" : "YETİŞKİN DANIŞAN"}
                </span>
        ...
        <div className={styles.processCard}>
          <div className={styles.processLeft}>
            ...
            <span className={styles.processSubtitle}>
              {completedAppointments} tamamlanan seans • {detail.appointments.length} toplam kayıt
            </span>
          </div>
          <div className={styles.processSteps}>
            ...
            <div className={completedAppointments >= 1 ? styles.stepActive : styles.stepInactive}>
              <span>{completedAppointments >= 1 ? "✓" : "○"}</span>
              <span>Müdahale</span>
            </div>
            <div className={completedAppointments >= 4 ? styles.stepActive : styles.stepInactive}>
              <span>{completedAppointments >= 4 ? "✓" : "○"}</span>
              <span>İzleme</span>
            </div>
            <div className={detail.status === "INACTIVE" ? styles.stepActive : styles.stepInactive}>
              <span>{detail.status === "INACTIVE" ? "✓" : "○"}</span>
        ...
        <div className={styles.tabs}>
          <button
            className={activeTab === "summary" ? styles.tabActive : styles.tabInactive}
          ...
          <button
            className={activeTab === "progress" ? styles.tabActive : styles.tabInactive}
          ...
          <button
            className={activeTab === "notes" ? styles.tabActive : styles.tabInactive}
          ...
          <button
            className={activeTab === "relations" ? styles.tabActive : styles.tabInactive}
      ...
      <div className={styles.lowerSection}>
          ...
          <div className={styles.grid}>
            <article className={styles.card}>
              ...
              <div className={styles.cardContent}>
                <div className={styles.field}>
                  ...
                  <span className={styles.value}>{detail.firstName}</span>
                </div>
                <div className={styles.field}>
                  <span className={styles.label}>Soyad</span>
                  <span className={styles.value}>{detail.lastName}</span>
                </div>
                <div className={styles.field}>
                  <span className={styles.label}>E-posta</span>
                  <span className={styles.valueBlue}>{detail.email ?? "—"}</span>
                </div>
                <div className={styles.field}>
            ...
            <article className={`${styles.card} ${styles.cardAccent}`}>
              <div className={styles.cardHeader}>
                ...
                <span className={styles.badgeMini}>{detail.nextAppointment ? "AKTİF" : "BOŞ"}</span>
              </div>
                  ...
                  <div className={styles.task}>
                    ...
                    <div className={styles.taskInfo}>
                      <span className={styles.taskName}>
                        {formatDateTime(detail.nextAppointment.startsAt)}
                      </span>
                      <span className={styles.taskTime}>
                        {appointmentStatusLabels[detail.nextAppointment.status] ??
                          detail.nextAppointment.status}{" "}
                        • {detail.nextAppointment.durationMinutesSnapshot} dk
                      </span>
                    </div>
                  </div>
                  <p className={styles.taskDesc}>
                    {detail.nextAppointment.requestNote ?? "Randevu notu bulunmuyor."}
                  </p>
                  <div className={styles.taskButtons}>
                    <Link className={styles.btnPrimary} href="/yonetim/randevular">
                      Randevuya git
                    </Link>
                    <button
            ...
            <article className={styles.card}>
              <div className={styles.cardHeader}>
                <h2 className={styles.cardTitle}>Kayıt Puanı</h2>
                <span className={styles.aiLabel}>CANLI VERİ</span>
              </div>
              <div className={styles.scoreContainer}>
                <div className={styles.scoreCircle}>
                  <span className={styles.scoreNumber}>{detail.score}</span>
                  <span className={styles.scoreGrade}>{grade}</span>
                </div>
                <div className={styles.scoreTrend}>
                  <span>{detail.score >= 70 ? "🟢 Düzenli" : "🟠 Geliştirilmeli"}</span>
                  <p className={styles.trendText}>
                    İletişim bilgileri, seans ve not kayıtlarından hesaplanır.
                  </p>
                </div>
              </div>
            ...
            <article className={styles.card}>
              ...
              <div className={styles.cardContent}>
                <div className={styles.field}>
                  ...
                  <span className={styles.value}>{activePlan?.name ?? "Plan yok"}</span>
                </div>
                <div className={styles.field}>
                  <span className={styles.label}>Seans</span>
                  <span className={styles.value}>
                    {activePlan
                      ? `${activePlan.sessionCount} × ${activePlan.sessionDurationMinutes} dk`
                      : "—"}
                  </span>
                </div>
                <div className={styles.field}>
            ...
            <article className={`${styles.card} ${styles.cardWarm}`}>
              ...
              <div className={styles.notesArea}>
                <p className={styles.notes}>
                  {latestNote?.note ?? "Henüz danışan notu eklenmedi."}
                </p>
                {latestNote ? (
                  <span className={styles.historyMeta}>
                    {formatDateTime(latestNote.createdAt)} •{" "}
                    {latestNote.createdBy.name ?? "Yönetici"}
                  </span>
                ) : null}
              </div>
            </article>
        ...
        {activeTab === "progress" ? (
          <div className={styles.grid}>
            <article className={`${styles.card} ${styles.fullWidth}`}>
              <div className={styles.cardHeader}>
                <h2 className={styles.cardTitle}>Randevu ve İlerleme Geçmişi</h2>
                <span className={styles.badgeMini}>{detail.appointments.length} KAYIT</span>
              </div>
              <div className={styles.historyList}>
                {detail.appointments.map((appointment) => (
                  <div className={styles.historyItem} key={appointment.id}>
                    ...
                    <span className={styles.historyMeta}>
                      {formatDateTime(appointment.startsAt)} • {appointment.durationMinutesSnapshot}{" "}
                      dk • {appointmentStatusLabels[appointment.status] ?? appointment.status}
                    </span>
                    {appointment.requestNote ? (
                      <span className={styles.notes}>{appointment.requestNote}</span>
                    ) : null}
                  </div>
                ))}
                {detail.appointments.length === 0 ? (
                  <p className={styles.empty}>Randevu geçmişi bulunmuyor.</p>
                ) : null}
              </div>
            </article>
          </div>
        ...
        {activeTab === "notes" ? (
          <div className={styles.grid}>
            <article className={`${styles.card} ${styles.fullWidth}`}>
              <div className={styles.cardHeader}>
                <h2 className={styles.cardTitle}>Danışan Notları</h2>
                <button
              ...
              <div className={styles.historyList}>
                {detail.notes.map((note) => (
                  <div className={styles.historyItem} key={note.id}>
                    ...
                    <span className={styles.historyMeta}>
                      {formatDateTime(note.createdAt)} • {note.createdBy.name ?? "Yönetici"}
                    </span>
                  </div>
                ))}
            ...
            <article className={styles.card}>
              ...
              <div className={styles.historyList}>
                ...
                {detail.guardians.length === 0 ? (
                  <p className={styles.empty}>Bağlı veli kaydı bulunmuyor.</p>
                ) : null}
              </div>
            </article>
            <article className={styles.card}>
              ...
              <div className={styles.historyList}>
                {detail.plans.map((plan) => (
                  <div className={styles.historyItem} key={plan.id}>
                    ...
                    <span className={styles.historyMeta}>
                      {plan.status} • {plan.sessionCount} seans •{" "}
                      {formatMoney(plan.totalAmountMinor, plan.currency)}
                    </span>
                  </div>
                ))}
                {detail.plans.length === 0 ? (
                  <p className={styles.empty}>Ödeme planı bulunmuyor.</p>
                ) : null}
              </div>
            </article>
            <article className={`${styles.card} ${styles.fullWidth}`}>
              ...
              <div className={styles.historyList}>
                {detail.financeEntries.map((entry) => (
                  <div className={styles.historyItem} key={entry.id}>
                    <strong>
                      {entry.type} • {formatMoney(entry.amountMinor, entry.currency)}
                    </strong>
                    <span className={styles.historyMeta}>
                      {formatDateTime(entry.occurredAt)} •{" "}
                      {entry.paymentMethod?.name ?? entry.plan?.name ?? "Genel hareket"}
                    </span>
                    {entry.note ? <span className={styles.notes}>{entry.note}</span> : null}
                  </div>
                ))}
                {detail.financeEntries.length === 0 ? (
                  <p className={styles.empty}>Finans hareketi bulunmuyor.</p>
                ) : null}
              </div>
            </article>
          </div>
      ...
      {modal === "edit" ? (
        <div className={styles.modalBackdrop} role="presentation">
          <div
            aria-labelledby="danisan-duzenle"
            aria-modal="true"
          ...
          >
            <div className={styles.modalHeader}>
              <div>
                ...
                <h2 className={styles.modalTitle} id="danisan-duzenle">
                  Danışan bilgilerini düzenle
                </h2>
              </div>
              <button className={styles.modalClose} onClick={() => setModal(null)} type="button">
                ×
              </button>
            </div>
            <form className={styles.modalForm} onSubmit={submitEdit}>
              <div className={styles.formGrid}>
                ...
                <label className={styles.formField}>
                  ...
                  <select defaultValue={detail.status} name="status">
                    <option value="PROSPECTIVE">Ön görüşme</option>
              ...
              <div className={styles.modalActions}>
                ...
                <button className={styles.modalButton} disabled={submitting} type="submit">
          ...
          <div aria-labelledby="not-ekle" aria-modal="true" className={styles.modal} role="dialog">
            <div className={styles.modalHeader}>
              <div>
                ...
                <h2 className={styles.modalTitle} id="not-ekle">
                  Danışan notu ekle
                </h2>
              </div>
              <button className={styles.modalClose} onClick={() => setModal(null)} type="button">
      ...
      {modal === "delete" ? (
        <div className={styles.modalBackdrop} role="presentation">
          ...
          >
            <div className={styles.modalHeader}>
              <div>
                <span className={styles.badge}>DURUM DEĞİŞİKLİĞİ</span>
                <h2 className={styles.modalTitle} id="danisan-pasif">
                  Danışanı pasife al
                </h2>
              </div>
              <button className={styles.modalClose} onClick={() => setModal(null)} type="button">
            ...
            <div className={styles.modalForm}>
              <p className={styles.notes}>
                {detail.firstName} {detail.lastName} kaydı silinmeyecek; geçmiş randevu ve ödeme
                bağlantıları korunarak pasif duruma alınacak.
              </p>
              {message ? <div className={styles.formMessage}>{message}</div> : null}
              <div className={styles.modalActions}>
                <button
