...
describe('Sales Hub client list adapter', () => {
  ...
  it('filters and sorts only the supplied live records', () => {
    ...
    expect(filterAndSortClientList([client, child], 'CHILD', '', 'updated').map((item) => item.id)).toEqual([
      'client-2',
    ]);
    expect(filterAndSortClientList([client, child], 'ALL', 'berfin', 'name').map((item) => item.id)).toEqual([
      'client-1',
    ]);
  });
});
