...
export default function OdemelerPage() {
  ...
  const payableInstallments =
    selectedPlan?.installments.filter((installment) => installment.state !== "PAID") ?? [];
  const activePlans = overview?.plans.filter((plan) => plan.status === "ACTIVE").length ?? 0;
  const overdueCount =
    overview?.plans.reduce(
      (total, plan) =>
  ...
  const openBalanceByCurrency = useMemo(() => {
    ...
    return (
      [...totals.entries()]
        .map(([currency, value]) => formatMoney(String(value), currency))
        .join(" + ") || "—"
    );
  }, [overview]);
      ...
      <div style={styles.page}>
        ...
        <section style={styles.summaryGrid}>
          <article style={styles.summaryCard}>
            ...
            <strong style={styles.summaryValue}>{activePlans}</strong>
            <small style={styles.summaryMeta}>Devam eden danışan planı</small>
          </article>
          <article style={styles.summaryCard}>
            <span style={styles.summaryLabel}>Gecikmiş taksit</span>
            <strong style={styles.summaryValue}>{overdueCount}</strong>
            <small style={styles.summaryMeta}>Vadesi geçmiş açık ödeme</small>
          </article>
          <article style={styles.summaryCard}>
            <span style={styles.summaryLabel}>Açık bakiye</span>
            <strong style={styles.balanceValue}>{openBalanceByCurrency}</strong>
            <small style={styles.summaryMeta}>Plan bazlı net bakiye</small>
          </article>
        </section>
        ...
        <section style={styles.listPanel}>
          <div style={styles.listHeader}>
            <h2 style={styles.listTitle}>Danışan planları</h2>
            <span style={styles.badge}>CANLI VERİ</span>
          </div>
          <div style={styles.planGrid}>
            {loading ? <p style={styles.empty}>Finans verileri yükleniyor...</p> : null}
            {!loading && (overview?.plans.length ?? 0) === 0 ? (
              <p style={styles.empty}>Henüz ödeme planı bulunmuyor.</p>
            ) : null}
              ...
              <article key={plan.id} style={styles.planCard}>
                <div style={styles.planHeader}>
                  ...
                  <span
                    style={{
                      ...styles.statusPill,
                      ...(plan.status === "ACTIVE" ? styles.statusActive : styles.statusNeutral),
                    }}
                  >
                    {plan.status}
                  </span>
                ...
                <div style={styles.planStats}>
                  <div>
                    <span>Toplam</span>
                    <strong>{formatMoney(plan.totalAmountMinor, plan.currency)}</strong>
                  </div>
                  <div>
                    <span>Açık bakiye</span>
                    <strong>{formatMoney(plan.balanceMinor, plan.currency)}</strong>
                  </div>
                  <div>
                ...
                <div style={styles.installmentList}>
                  {plan.installments.map((installment) => (
                    <div key={installment.id} style={styles.installmentRow}>
                      <div>
                        <strong>Taksit {installment.sequence}</strong>
                      ...
                      <div style={styles.installmentRight}>
                        ...
                        >
                          {installmentLabels[installment.state]}
                        </span>
                      </div>
                    </div>
                  ))}
                ...
                <div style={styles.cardActions}>
                  <button
                    disabled={
                      plan.status !== "ACTIVE" ||
                      plan.installments.every((item) => item.state === "PAID")
                    }
                    onClick={() => void openPayment(plan.id)}
                    style={styles.primaryButton}
                    type="button"
                  >
                    Ödeme kaydet
        ...
        <div style={styles.modalBackdrop} role="presentation">
          <div aria-labelledby="plan-olustur" aria-modal="true" role="dialog" style={styles.modal}>
            <div style={styles.modalHeader}>
              <div>
                <span style={styles.kicker}>YENİ PLAN</span>
                <h2 id="plan-olustur" style={styles.modalTitle}>
                  Ödeme planı oluştur
                </h2>
              </div>
              <button onClick={() => setModal(null)} style={styles.closeButton} type="button">
                ×
              </button>
            </div>
            <form onSubmit={createPlan} style={styles.form}>
              <div style={styles.formGrid}>
                ...
                <label style={styles.field}>
                  <span>Plan adı</span>
                  <input defaultValue="Terapi Paketi" name="name" required style={styles.input} />
                </label>
                <label style={styles.field}>
                ...
                <label style={styles.field}>
                  ...
                  <select defaultValue="45" name="sessionDurationMinutes" style={styles.input}>
                ...
                <label style={styles.field}>
                  ...
                  <select defaultValue="EUR" name="currency" style={styles.input}>
              ...
              <div style={styles.modalActions}>
                <button onClick={() => setModal(null)} style={styles.secondaryButton} type="button">
                  Vazgeç
                </button>
      ...
      {modal === "payment" && selectedPlan ? (
        <div style={styles.modalBackdrop} role="presentation">
          <div aria-labelledby="odeme-kaydet" aria-modal="true" role="dialog" style={styles.modal}>
            <div style={styles.modalHeader}>
              <div>
                <span style={styles.kicker}>ÖDEME</span>
                <h2 id="odeme-kaydet" style={styles.modalTitle}>
                  {selectedPlan.client.firstName} {selectedPlan.client.lastName}
                </h2>
              </div>
              <button onClick={() => setModal(null)} style={styles.closeButton} type="button">
                ×
            ...
            <form onSubmit={recordPayment} style={styles.form}>
              <div style={styles.formGrid}>
                ...
                <label style={styles.field}>
                  <span>Taksit</span>
                  <select name="installmentId" required style={styles.input}>
                    <option value="">Taksit seçin</option>
                    {payableInstallments.map((installment) => (
                      <option key={installment.id} value={installment.id}>
                        Taksit {installment.sequence} •{" "}
                        {formatMoney(
                          String(
                            Number(installment.amountDueMinor) -
                              Number(installment.paidAmountMinor),
                          ),
                ...
                <label style={styles.field}>
                  ...
                  <select name="paymentMethodId" required style={styles.input}>
                ...
                <label style={styles.field}>
                  <span>Referans</span>
                  <input name="externalReference" style={styles.input} />
                </label>
              </div>
              <label style={styles.field}>
                <span>Not</span>
                <textarea maxLength={500} name="note" style={styles.textarea} />
              </label>
              {message ? <div style={styles.message}>{message}</div> : null}
              <div style={styles.modalActions}>
                <button onClick={() => setModal(null)} style={styles.secondaryButton} type="button">
