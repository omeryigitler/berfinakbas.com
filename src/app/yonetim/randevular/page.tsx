...
export default function RandevularPage() {
  ...
  const loadAppointments = useCallback(async () => {
    ...
    try {
      const [pendingResponse, confirmedResponse] = await Promise.all([
        fetch("/api/admin/appointments?status=REQUESTED,PENDING_REVIEW&take=100", {
          cache: "no-store",
        }),
        fetch("/api/admin/appointments?status=CONFIRMED,RESCHEDULE_PROPOSED&take=100", {
          cache: "no-store",
        }),
      ]);
  ...
  const selectedService = useMemo(
    () =>
      prerequisites?.services.find((service) => service.id === selectedServiceId) ??
      prerequisites?.services[0] ??
      null,
  ...
  async function setupPrerequisites() {
    ...
    try {
      ...
      setMessage(
        error instanceof Error ? error.message : "Varsayılan randevu ayarları oluşturulamadı.",
      );
    } finally {
      setSubmitting(false);
    }
      ...
      const response = await fetch("/api/admin/appointments", {
        body: JSON.stringify({
          ...
          durationMinutes: Number(
            formData.get("durationMinutes") ?? selectedService.defaultDurationMinutes,
          ),
          guardianId: null,
  ...
  const pendingCount = appointments.filter((appointment) =>
    ["REQUESTED", "PENDING_REVIEW"].includes(appointment.status),
  ).length;
    ...
    <>
      <div style={styles.page}>
        <section style={styles.headerCard}>
          ...
          <div style={styles.headerActions}>
            ...
            <button onClick={() => void openCreate()} style={styles.primaryButton} type="button">
              Yeni randevu
            </button>
          </div>
        </section>
        ...
        <section style={styles.summaryGrid}>
          <article style={styles.summaryCard}>
            <span style={styles.summaryLabel}>Açık talep</span>
            <strong style={styles.summaryValue}>{pendingCount}</strong>
            <small style={styles.summaryMeta}>Karar veya inceleme bekliyor</small>
          </article>
          <article style={styles.summaryCard}>
            <span style={styles.summaryLabel}>Onaylı</span>
            <strong style={styles.summaryValue}>{confirmedCount}</strong>
            <small style={styles.summaryMeta}>Planlanan aktif seans</small>
          </article>
          <article style={styles.summaryCard}>
            <span style={styles.summaryLabel}>Toplam görünür</span>
            <strong style={styles.summaryValue}>{appointments.length}</strong>
            <small style={styles.summaryMeta}>Açık ve yaklaşan kayıt</small>
          </article>
        </section>
        ...
        <section style={styles.listPanel}>
          <div style={styles.listHeader}>
            <h2 style={styles.listTitle}>Randevu akışı</h2>
            <span style={styles.badge}>CANLI VERİ</span>
          </div>
          <div style={styles.list}>
            {loading ? <p style={styles.empty}>Randevular yükleniyor...</p> : null}
            {!loading && appointments.length === 0 ? (
              <p style={styles.empty}>Açık veya yaklaşan randevu bulunmuyor.</p>
            ) : null}
            {appointments.map((appointment) => (
              <article key={appointment.id} style={styles.appointmentCard}>
                ...
                <div style={styles.appointmentMain}>
                  ...
                  <small>
                    {appointment.publicReference} • {appointment.locationTypeSnapshot}
                  </small>
                </div>
                <div style={styles.appointmentMeta}>
                  ...
                  {appointment.duplicateReview.status === "PENDING" ? (
                    <small style={styles.warning}>Olası mükerrer kayıt</small>
                  ) : null}
                  {appointment.status === "REQUESTED" ? (
                    <div style={styles.rowActions}>
                      <button
                        disabled={submitting}
                        onClick={() =>
                          void transitionStatus(appointment.id, "PENDING_REVIEW", "ADMIN_REVIEW")
                        }
                      ...
                      {pendingAppointmentId === appointment.id ? (
                        <small style={styles.rowPending}>İşleniyor…</small>
                      ) : null}
                    </div>
                  ) : null}
                  {["PENDING_REVIEW", "RESCHEDULE_PROPOSED"].includes(appointment.status) ? (
                    <div style={styles.rowActions}>
                      <button
                        disabled={submitting}
                        onClick={() =>
                          void transitionStatus(appointment.id, "CONFIRMED", "ADMIN_CONFIRMED")
                        }
                      ...
                      <button
                        disabled={submitting}
                        onClick={() =>
                          void transitionStatus(appointment.id, "REJECTED", "ADMIN_REJECTED")
                        }
      ...
      {createOpen ? (
        <div style={styles.modalBackdrop} role="presentation">
          <div
            aria-labelledby="randevu-olustur"
            aria-modal="true"
            role="dialog"
            style={styles.modal}
          >
            <div style={styles.modalHeader}>
              <div>
                <span style={styles.kicker}>YENİ RANDEVU</span>
                <h2 id="randevu-olustur" style={styles.modalTitle}>
                  Randevu oluştur
                </h2>
              </div>
              <button onClick={() => setCreateOpen(false)} style={styles.closeButton} type="button">
                ×
              </button>
            </div>

            {prerequisites &&
            (prerequisites.services.length === 0 || prerequisites.practitioners.length === 0) ? (
              <div style={styles.setupCard}>
                <strong>Randevu için varsayılan hizmet veya terapist kaydı eksik.</strong>
                <span>
                  Mevcut yönetici hesabına bağlı ilk görüşme altyapısı otomatik oluşturulabilir.
                </span>
                <button
                  disabled={submitting}
                  onClick={() => void setupPrerequisites()}
                  style={styles.primaryButton}
                  type="button"
                >
                  {submitting ? "Hazırlanıyor..." : "Randevu altyapısını hazırla"}
                </button>
              </div>
            ) : null}

            {prerequisites &&
            prerequisites.services.length > 0 &&
            prerequisites.practitioners.length > 0 ? (
              <form onSubmit={createAppointment} style={styles.form}>
                <div style={styles.formGrid}>
                  <label style={styles.field}>
                    <span>Danışan</span>
                    <select name="clientId" required style={styles.input}>
                      <option value="">Danışan seçin</option>
                      {prerequisites.clients.map((client) => (
                  ...
                  <label style={styles.field}>
                    <span>Terapist</span>
                    <select name="practitionerId" required style={styles.input}>
                      <option value="">Terapist seçin</option>
                      {prerequisites.practitioners.map((practitioner) => (
                  ...
                  <label style={styles.field}>
                    ...
                    >
                      {prerequisites.services.map((service) => (
                  ...
                  <label style={styles.field}>
                    <span>Süre</span>
                    <select
                      defaultValue={selectedService?.defaultDurationMinutes ?? 15}
                  ...
                  <label style={styles.field}>
                    <span>Konum</span>
                    <select
                      defaultValue={selectedService?.locationType ?? "HYBRID"}
                      key={`location-${selectedService?.id}`}
                      name="locationType"
                      style={styles.input}
                    >
                ...
                <label style={styles.field}>
                  ...
                  <textarea maxLength={500} name="requestNote" style={styles.textarea} />
                </label>
...
const styles = {
  ...
  headerCard: {
    ...
    gap: 20,
    border: "1px solid rgba(50,49,48,.12)",
    borderRadius: 28,
    background: "linear-gradient(135deg,#fff 0%,#faf9f5 68%,#efffc3 100%)",
    padding: 24,
  },
  kicker: {
    display: "inline-flex",
  ...
  secondaryButton: {
    minHeight: 38,
    border: "1px solid rgba(50,49,48,.15)",
    borderRadius: 999,
    background: "#fff",
    padding: "0 16px",
    color: "#4e4a46",
  ...
  listHeader: {
    ...
    alignItems: "center",
    justifyContent: "space-between",
    borderBottom: "1px solid rgba(50,49,48,.08)",
    paddingBottom: 12,
  },
  ...
  rejectButton: {
    minHeight: 28,
    border: "1px solid rgba(50,49,48,.2)",
    borderRadius: 999,
    background: "#fff",
    padding: "0 12px",
    color: "#9a3412",
  ...
  modal: {
    ...
    background: "#fff",
    padding: 22,
    boxShadow: "0 28px 80px rgba(5,5,5,.22)",
  },
  modalHeader: {
    display: "flex",
    ...
    justifyContent: "space-between",
    gap: 16,
    borderBottom: "1px solid rgba(50,49,48,.1)",
    paddingBottom: 14,
  },
  ...
  closeButton: {
    ...
    height: 32,
    placeItems: "center",
    border: "1px solid rgba(50,49,48,.14)",
    borderRadius: "50%",
    background: "#fff",
    cursor: "pointer",
  },
