...
export async function GET(request: Request) {
  ...
  if (!parsed.success)
    return NextResponse.json({ error: "Yönetim modülü geçersiz." }, { status: 400 });
...
export async function POST(request: Request) {
  ...
  if (!parsed.success)
    return NextResponse.json(
      { error: "Yönetim işlemi geçersiz.", issues: parsed.error.flatten() },
      { status: 400 },
    );
  return writeManagementAction(
    session,
    parsed.data,
    getSafeCorrelationId(request.headers.get("x-correlation-id")),
  );
}
