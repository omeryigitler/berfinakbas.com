...
export function ReportsView({ data, selectedItemId }: ModuleViewsProps) {
  ...
  return rows.length === 0 ? <EmptyState title="Rapor verisi yok" text="Bu raporu görüntülemek için gerekli yetki veya kayıt bulunmuyor." /> : (
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
      {rows.map((row: any, index: number) => <Metric key={`${selectedItemId}-${index}`} title={row.status ?? row.type ?? row.currency ?? 'Kayıt'} value={row._count?._all ?? row._sum?.amountMinor ?? 0} text={row.currency ? `${row.currency} · ${row.type}` : 'Canlı veritabanı sonucu'} />)}
    </div>
  );
}
...
export function UsersView({ data, selectedItemId, filter, sortDirection, refresh, notify }: ModuleViewsProps) {
  const users = useMemo(() => {
    ...
    return [...(data?.users??[])].filter((user)=>normalized(`${user.name} ${user.email} ${user.status} ${user.roles?.map((r:any)=>r.role.name).join(' ')}`).includes(query)).sort((a,b)=>text(a.name||a.email).localeCompare(text(b.name||b.email),'tr')*(sortDirection==='asc'?1:-1));
  },[data,filter,sortDirection]);
  async function toggle(user:any){
    try{await postAction({action:'UPDATE_USER_STATUS',userId:user.id,status:user.status==='ACTIVE'?'SUSPENDED':'ACTIVE'});notify({kind:'success',title:'Kullanıcı durumu güncellendi',message:`${user.name||user.email} hesabı güncellendi.`});await refresh();}catch(error){notify({kind:'error',title:'Kullanıcı güncellenemedi',message:error instanceof Error?error.message:'Beklenmeyen hata.'});}
  }
  if(selectedItemId==='roller'){
    return <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">{['SUPER_ADMIN','THERAPIST','ASSISTANT','FINANCE','DEVELOPER'].map((role)=><article key={role} className="rounded-[2rem] border border-black/[0.07] bg-white/88 p-5"><ShieldCheck className="h-5 w-5" /><h2 className="mt-4 text-[12px] font-black">{role}</h2><p className="mt-2 text-[9px] font-semibold text-gray-400">Rol atamaları kullanıcı kayıtları üzerinden okunur. Yeni rol yetkileri kod seviyesindeki izin matrisiyle korunur.</p></article>)}</div>;
  }
  if(selectedItemId==='giris-gecmisi'){
    return <section className="overflow-hidden rounded-[2rem] border border-black/[0.07] bg-white/88">{users.map((user,index)=><article key={user.id} className={`grid grid-cols-[1fr_220px_auto] gap-4 px-5 py-4 ${index>0?'border-t border-black/[0.05]':''}`}><div><strong className="block text-[11px] font-black">{user.name||'İsimsiz kullanıcı'}</strong><span className="mt-1 block text-[9px] font-semibold text-gray-400">{user.email}</span></div><span className="text-[10px] font-bold">{formatDate(user.lastLoginAt,true)}</span><StatusPill value={user.status}/></article>)}</section>;
  }
  return <section className="overflow-hidden rounded-[2rem] border border-black/[0.07] bg-white/88">{users.map((user,index)=><article key={user.id} className={`grid grid-cols-[1fr_240px_auto] items-center gap-4 px-5 py-4 ${index>0?'border-t border-black/[0.05]':''}`}><div><strong className="block text-[11px] font-black">{user.name||'İsimsiz kullanıcı'}</strong><span className="mt-1 block text-[9px] font-semibold text-gray-400">{user.email}</span></div><span className="text-[9px] font-bold text-gray-500">{user.roles?.map((r:any)=>r.role.name).join(', ')||'Rol yok'}</span><button type="button" disabled={user.id===data.currentUserId} onClick={()=>void toggle(user)} className={`rounded-full px-3 py-2 text-[8px] font-black disabled:opacity-40 ${user.status==='ACTIVE'?'border border-amber-200 bg-amber-50 text-amber-700':'border border-emerald-200 bg-emerald-50 text-emerald-700'}`}>{user.status==='ACTIVE'?'Askıya al':'Aktifleştir'}</button></article>)}</section>;
}

export function SettingsView({ data, selectedItemId, refresh, notify }: ModuleViewsProps) {
  const keyMap: Record<string,string>={isletme:'BUSINESS_PROFILE',randevu:'BOOKING_RULES',bildirimler:'NOTIFICATION_SETTINGS',kvkk:'PRIVACY_SETTINGS',gorunum:'APPEARANCE_SETTINGS'};
